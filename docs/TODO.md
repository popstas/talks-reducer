...
- [ ] run server + GUI together: `talks-reducer-gui --server` currently launches only the tray-managed server with no desktop window. Allow starting both the server and the desktop GUI together from the macOS pip app (e.g. a combined flag or have `--server` also open the GUI).
- [ ] macOS tray icon: the pystray system-tray icon does not appear on macOS when running `--server`. Make the tray icon work on macOS (or provide a documented working fallback).
