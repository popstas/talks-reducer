...
- **Cut video** — a checkbox (available in both Simple and Advanced layouts)
  ...
  still via Pillow (`ImageTk`). When the box is on, the GUI emits the same
  keep-range used by the `--cut-start`/`--cut-end` CLI flags as
  ...
  path; when off the trim args are omitted. The enabled flag plus the
