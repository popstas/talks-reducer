...
- `_set_status` also calls `_ring_completion_bell()`, which rings Tk's `root.bell()` on a success or `Error` status and stays silent for `Aborted` and every non-terminal status. It is cross-platform (unlike the taskbar) and `suppress(Exception)`-guarded, since a display without a bell raises rather than staying quiet.
