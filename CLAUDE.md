...
- `_is_window_focused()` wraps `root.focus_displayof()` — `None` for another app's window, and it *raises* when the focused window is one Tk cannot name. Both mean "not us", and a raise reports unfocused so an outcome is announced rather than silently swallowed. Both `_clear_taskbar_if_focused()` and the bell gate on it.
- The refocus clear is bound to **both** `<FocusIn>` and `<Activate>` on `root`: `<FocusIn>` is delivered to whichever widget takes the keyboard focus, while `<Activate>` reaches the toplevel when Windows/macOS makes the window active. `TaskbarProgress.on_focus()` is idempotent, so a double delivery is harmless.
